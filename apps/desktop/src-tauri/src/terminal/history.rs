...
use crate::terminal::ghostty::{GhosttyTerminalState, TerminalSearchMatch, TerminalSearchResult};
...
/// Search the replayed durable transcript at the given width without returning
/// every rendered row to the frontend.
pub fn history_search(
    ...
    state.search(query, case_sensitive, max_matches)
}

/// Search transcript segments in stitched history coordinates.
pub fn history_search_segments(
    transcripts: &[Vec<u8>],
    cols: u16,
    rows: u16,
    query: &str,
    ...
    let transcripts = replay_segments(transcripts);
    if transcripts.is_empty() {
        return history_search(&[], cols, rows, query, case_sensitive, max_matches, colors);
    }

    let mut matches = Vec::<TerminalSearchMatch>::new();
    let mut total = 0_usize;
    let mut total_rows = 0_usize;
    for transcript in transcripts {
        let remaining = max_matches.saturating_sub(matches.len());
        let result = history_search(
            transcript,
            cols,
            rows,
            query,
        ...
        for mut match_ in result.matches {
            match_.row = match_.row.saturating_add(total_rows);
            matches.push(match_);
        }
        total = total.saturating_add(result.total);
        total_rows = total_rows.saturating_add(result.total_rows.max(1));
    }

    let capped = total > matches.len();
...
/// Search the replayed transcript and return a paintable history window from
/// the same Ghostty replay. The window is centered around the first match when
/// possible, or starts at the top for a no-match search.
#[cfg(test)]
pub fn history_search_window(
    transcript: &[u8],
    cols: u16,
    surface_rows: u16,
    query: &str,
    ...
    max_matches: usize,
    colors: TerminalThemeColors,
) -> Result<(TerminalSearchResult, TerminalFrame)> {
    let window_rows = history_window_rows(surface_rows, usize::MAX);
    let mut state =
        GhosttyTerminalState::new(cols, surface_rows.max(1), HISTORY_MAX_SCROLLBACK_BYTES)?;
    state.set_default_colors(colors.foreground, colors.background);
    if !transcript.is_empty() {
    ...
    let search = state.search(query, case_sensitive, max_matches)?;
    let target_row = search.matches.first().map(|match_| match_.row).unwrap_or(0);
    let start_row =
        plan_history_window_start(target_row, surface_rows, window_rows, search.total_rows);
    let frame = collect_history_window(&mut state, start_row, window_rows)?;
    Ok((search, frame))
}

/// Search transcript segments and return a paintable stitched history window
/// from the same segmented replay model.
pub fn history_search_window_segments(
    transcripts: &[Vec<u8>],
    cols: u16,
    surface_rows: u16,
    query: &str,
    ...
    let window_rows = history_window_rows(surface_rows, usize::MAX);
    let search = history_search_segments(
        transcripts,
        cols,
        surface_rows,
        query,
...
mod tests {
    ...
    #[test]
    fn segmented_search_offsets_matches_by_prior_runs() {
        let transcripts = vec![numbered_lines(8), b"needle\r\n".to_vec()];
        let first_rows = history_total_rows(&transcripts[0], 80, 4).unwrap();
        let result = history_search_segments(
            &transcripts,
            80,
            4,
            "needle",
        ...
        assert_eq!(result.matches[0].row, first_rows);
        assert_eq!(
            result.total_rows,
            history_total_rows_segments(&transcripts, 80, 4).unwrap()
        );
    }

    #[test]
    fn searches_replayed_transcript_without_extracting_full_history() {
        let transcript = numbered_lines(80);
        let result = history_search(
            &transcript,
            80,
        ...
        assert_eq!(
            result.total_rows,
            history_total_rows(&transcript, 80, 6).unwrap()
        );
        assert_eq!(result.total, 11);
        assert!(result.capped);
        assert_eq!(result.matches.len(), 3);
        assert!(result.matches[0].line_text.starts_with("line7"));
        assert!(result.matches[0].row >= 7);
    }

    #[test]
    fn search_maps_wide_cells_to_replayed_cell_columns() {
        let result = history_search(
            "A界B\r\n".as_bytes(),
            20,
            4,
        ...
        assert_eq!(result.total, 1);
        assert_eq!(result.matches[0].start_col, 3);
        assert_eq!(result.matches[0].end_col, 4);
        assert_eq!(result.matches[0].line_text, "A界B");
    }

    #[test]
    fn searches_and_extracts_match_window_from_one_replay() {
        let transcript = numbered_lines(100);
        let (search, frame) = history_search_window(
            &transcript,
            80,
            10,
            "line50",
        ...
        assert_eq!(search.matches[0].row, 50);
        assert_eq!(frame.scrollback.viewport_offset, 40);
        assert!(
            row_text(&frame, 0).starts_with("line40"),
            "row 0 was {:?}",
            row_text(&frame, 0)
        );
    }
    ...
    #[test]
    fn search_window_no_match_returns_top_context() {
        let transcript = numbered_lines(40);
        let (search, frame) = history_search_window(
            &transcript,
            80,
            8,
