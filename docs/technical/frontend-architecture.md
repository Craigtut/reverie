...
- `apps/desktop/web/App.tsx` stays deliberately thin: it only routes to the dev surfaces or mounts `<WorkspaceShell />`. The product shell, workspace/project/focus/session navigation, create-focus/create-session flows, metrics UI, and Tauri command/event subscriptions live in `WorkspaceShell` and the hooks it composes, never in `App.tsx`. App state is held in focused **Zustand** stores under `apps/desktop/web/store/` (navigation, ui, palette, activity, shell, terminal, connectionPanel, overlay), subscribed via selectors. The selected-session launch path uses stable runtime commands/events: `workspace_shell`, `create_focus`, `create_session`, `start_session`, `terminal_stream_started`, `terminal_frame`, `terminal_exit`, and `terminal_failed`.
- Persistence is SQLite, behind the `WorkspaceRepository` trait (`packages/reverie-core/src/repository.rs`), implemented by the `reverie-persistence` crate. The seeded snapshot is only the first-run bootstrap (`ensure_seeded` is idempotent); subsequent workspace/focus/session changes are incremental by-id writes through the repository. There is no `app_shell.rs` JSON document store.
...
2. Keep building on the SQLite `WorkspaceRepository` (incremental by-id writes, ordered migrations) for durable session status updates and new domain records.
