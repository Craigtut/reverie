...
`libghostty` is the history store. When the frontend scrolls up and runs low on mirrored rows, it asks for more; the backend reads that range from `libghostty`'s buffer and returns it. The backend must read arbitrary rows from the buffer **without moving `libghostty`'s viewport or disturbing the running terminal's active area**, where the live process keeps writing. The clean separation between the displayed viewport and the active area is what makes this safe; see [`libghostty-history-limits.md`](libghostty-history-limits.md) for how that works and its limits.

This is the line from D6: `libghostty` serves rows, the frontend drives the viewport. The backend never decides what is on screen.

Two practical constraints from `libghostty` shape how this is built (details in [`libghostty-history-limits.md`](libghostty-history-limits.md)). First, the live viewport uses `libghostty`'s fast render-state path, while arbitrary history ranges use a separate read path that traverses the scrollback and is comparatively expensive, and is explicitly not for a render loop. So the backend resolves a requested band once, copies the rows out immediately under the terminal lock, and caches them; it never reads history per cell per frame. Second, the alternate screen has no scrollback (full-screen TUIs), so history serving and the scroll-back affordance are suppressed while a session is on the alternate screen, and re-seeded when it returns to the primary screen.
