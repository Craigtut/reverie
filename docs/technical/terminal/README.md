...
- **`libghostty` owns the history; the frontend drives scrolling.** The backend serves ranges of rows from `libghostty`'s buffer when the frontend asks; the frontend never hands scrolling to the backend. See [`decisions.md`](decisions.md) (D6).
...
- [`libghostty-history-limits.md`](libghostty-history-limits.md) — how far back `libghostty`'s buffer reaches, how it stores and serves rows, and the reflow caveat.
