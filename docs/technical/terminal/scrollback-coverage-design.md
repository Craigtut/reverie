...
Verified against the installed binding (`libghostty-vt` 0.2.0 + `-sys`) and the vendored Ghostty source it builds:
...
- **Tracked refs are available only inside the backend worker.** `libghostty-vt` 0.2 exposes `track_grid_ref` and point conversion APIs. Reverie uses them for one-shot resize anchoring, where the worker creates the ref, performs resize, resolves it, and discards it. The frontend still cannot hold a live ref across IPC, so tracked refs do not replace stable row ids for the scroll cache.
- **No exact eviction count is exposed.** There is still no trim/eviction signal or monotonic "lines ever" counter. That is why `lines_evicted` remains backend bookkeeping, best-effort at the byte cap, and exact below it.
...
- **Reflow (resize):** rewraps rows, so the *content behind* ids changes even though the id space stays monotonic. We do not remap ids one-to-one (neither does WezTerm); we **re-seed**: bump the generation, drop the cache, and re-fetch. When scrolled back on the primary screen, the frontend sends the top visible stable id and the backend uses a `libghostty-vt` 0.2 tracked ref to preserve that position across reflow. Reflowing a redraw-heavy TUI's hard-wrapped scroll-back is still inherently lossy regardless of anchoring. So trim is handled by the id (no re-seed); reflow is handled by re-seed plus a one-shot position anchor.

**The one honest residual.** Keeping `lines_evicted` exact requires knowing how many rows libghostty evicted, and 0.2.0 still emits no eviction signal; eviction happens silently inside the core, and coalesced append+prune within one write batch can mask the count in `total_rows` deltas. So `lines_evicted` is **exact below the cap (zero)** and **best-effort at the cap**. The only user-visible effect is in one corner: scrolled back *while* output is actively flooding past the dial, the anchor can **drift** (content stays correct; the scroll position slides) until it re-anchors. This is graceful, never wrong content and never a livelock, far smaller than position-keying's desync-on-every-trim. It can be retired if libghostty exposes an eviction count, or if a tracked-ref spike gives us an exact oldest-row floor without hot-path cost.
...
- **At-cap id drift (the residual)** is graceful (content correct, self-healing) and becomes exact when libghostty exposes an eviction count, or if a tracked-ref spike gives us an exact oldest-row floor without hot-path cost. The StableRowIndex architecture adopts either source with no wire rework. Tracked in [`decisions.md`](decisions.md) D8.
