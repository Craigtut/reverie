...
- **By-position reads exist.** `terminal.grid_ref(Point) -> GridRef`, `Point` ∈ `Active | Viewport | Screen | History` with `{x, y}` where `y` may exceed a page for `Screen`/`History`. `GridRef::row()` yields the cells. So we read any history row by position **without moving the viewport** (the current `read_rows` viewport-pin excursion can be deleted). Caveats: `Screen`/`History` lookups traverse the page list (`O(pages)`, not for the render loop), and a `GridRef` is volatile (valid only until the next mutation), so read under the worker lock and copy out immediately.
...
- Rewrite `read_rows` to take an **id range**, map to buffer position (`pos = id - lines_evicted`), clamp to the live range, and read via `grid_ref(Point::Screen{ y: pos })` under the lock, copying cells out immediately. This deletes the viewport-pin excursion and its restore/error handling.
- Keep the per-session **generation** and bump it on resize, full clear, and alt-screen toggle (directly observable). Trim does **not** bump it. Stamp generation + `[oldest_id, newest_id]` on every frame and every band.

**Wire (`wire-protocol.md`):**
- Frames and bands carry `generation` + `[oldest_id, newest_id]` (the retained range) alongside the existing viewport offset + rows. History requests address an **id range**. This is a small additive tightening (the existing generation marker generalizes; the range is two new u64s), not a new protocol shape.
...
- **S1. Scroll up beyond the mirror.** Window nears the band edge -> prefetch a band by id range via `grid_ref` -> merge as Confirmed -> persists.
...
- **Phase A (backend identity + reads):** add `lines_evicted` tracking + `[oldest_id, newest_id]`; rewrite `read_rows` to id-range + `grid_ref` (drop the pin-move/restore); keep the generation, bump on resize/clear/alt. Tests: `id == position` below the cap; `oldest_id` advances on eviction; `read_rows` returns the right band by id; generation bumps on each event.
- **Phase B (frontend coverage by id):** key the cache/coverage by stable id with per-id state (Confirmed(generation)/Provisional/Absent); live ingest + fetch merge + reconciliation + live-wins-overlap; trim drops below `oldest_id` without re-seed; generation bump re-seeds + re-anchors; hysteresis; id-distance eviction. Tests: every scenario in section 6.
- **Phase C (wire/identity tightening):** stamp generation + retained range on frames + bands; id-range fetch requests; drop stale-generation responses.
...
- **`grid_ref` cost** (`O(pages)` deep) is mitigated by fetching bands and caching by id (each region read about once); never per frame.
