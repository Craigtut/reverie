...
1. **Scroll performance (fixed in code, pending the manual pass).** Scrolling up stalled: a little moved, then it hung while a one-screen band loaded, repeatedly. Root cause was a 3-row overscan plus a reactive one-screen fetch, and a paint loop that froze on a miss until the band arrived. Fix: prefetch a large aligned band (`HISTORY_PREFETCH_LEAD_ROWS`/`HISTORY_PREFETCH_ALIGN_ROWS` in `frameModel.ts`) ahead of the viewport when scrolled back, and paint blank placeholder rows on a miss so the gesture never freezes (it fills async). Mirrors how remote terminal clients (WezTerm's mux client) stay smooth: large warm cache plus non-blocking placeholders.
2. **Claude Code (Ink) resize jumble (fixed in code, pending the manual pass).** Scrolling back after a width resize painted stale old-width rows. Root cause: the frontend preserved its cached scrollback across a width change instead of re-fetching the reflowed rows. Fix: on a width change drop the scrolled-back cache and keep only the viewport as an anti-flicker bridge, so scroll-back re-fetches reflowed rows from `libghostty` (the WezTerm `make_all_stale`-on-resize discipline). Ink showed it worst because it redraws in place on SIGWINCH and miscounts wrapped height, churning the backend buffer; plain append output (Cortex) degraded gracefully and never triggered it.
3. **Interactive in-app manual pass + the branch merge decision.** macOS WKWebView cannot be WebDriver-automated, so the visual/interaction pass (scroll feel through long history, Ink resize while scrolled back, input latency, many concurrent sessions) must be done by hand in the running desktop app. The merge of `refactor/terminal-overhaul` into `main` waits on that pass confirming items 1 and 2 feel right. Watch for a brief flash during the resize gesture itself (the stale-shape transient before the re-seed frame lands) and for any per-band stall on very fast flicks; both are tunable follow-ups if they show.
